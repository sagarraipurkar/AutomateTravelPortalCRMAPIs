...
public class AgencyEndPoints {
	...
	public static Response CreateAgency() throws FileNotFoundException {
		...
		File f = new File("C:\\Eclipse New\\NewWorkSpace\\TravelCrmApiAutomation\\testdata\\CreateAgencyTestData.json");
		FileReader fr = new FileReader(f);
		JSONTokener jsonToken = new JSONTokener(fr);
		JSONObject data = new JSONObject(jsonToken);
		Response response = given().header("Authorization", "Bearer " + Token).contentType(ContentType.JSON).body(data)
				.when().post(Post_url);
		return response;
	}
	...
	public static Response GetAgency() {
		...
		Response response = given().header("Authorization", "Bearer " + Token).pathParam("agencyId", 1)
				.contentType(ContentType.JSON).when().get(get_url);
		return response;
	}
