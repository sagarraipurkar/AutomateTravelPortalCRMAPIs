...
public class AgentEndPoints {
	...
	public static Response CreateAgent() throws FileNotFoundException {
		...
		Response AgentResponse = given().header("Authorization", "Bearer " + Token)
				.header("Content-Type", "application/json").body(data.toString()).when().post(Post_agent);
		return AgentResponse;
	}

	public static Response GetAgent() {
		String get_agent = getURL().getString("get_agent");
		Response response = given().header("Authorization", "Bearer " + Token).pathParam("agentId", 1)
				.contentType(ContentType.JSON).when().get(get_agent);
		return response;
	}
...
//	}
//
//	public static Response getAgentList(String AgencyId, String Payload) {
//		String Post_agent_list = getURL().getString("Post_agent_list");
//		Response response = given().header("Authorization", "Bearer " + Token).pathParam("agencyId", AgencyId)
//				.contentType(ContentType.JSON).accept(ContentType.TEXT).when().put(Post_agent_list);
//		return response;
//	}
